...
export function requireDepartmentAccess(options: DepartmentAccessOptions = {}) {
  const {
  ...
  return async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    ...
    try {
      ...
      const targetDepartmentId =
        ...
        req.params.id ||
        (req.query[departmentIdParam] as string) ||
        req.body?.[departmentIdParam];

      // Extract target employee ID from params, query, or body
      const targetEmployeeId =
        req.params[employeeIdParam] ||
        req.params.id ||
        (req.query[employeeIdParam] as string) ||
