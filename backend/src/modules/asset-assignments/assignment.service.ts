...
export class AssignmentService {
  constructor(private repo: AssignmentRepository = assignmentRepository) {}
  ...
  async getAssignments(query: AssignmentQueryDTO) {
    ...
    if (query.isCurrent === 'true') isCurrent = true;
    if (query.isCurrent === 'false') isCurrent = false;
  ...
  async assignAsset(data: AssignAssetDTO) {
    ...
    if (asset.status !== 'AVAILABLE') {
      throw new AppError(
        `Asset '${asset.assetCode}' cannot be assigned because its current status is ${asset.status}. Only AVAILABLE assets can be assigned.`,
        422,
      );
    }
    ...
    if (!employee.isActive) {
      throw new AppError(
        `Employee '${employee.firstName} ${employee.lastName}' is inactive and cannot be assigned assets.`,
        422,
      );
    }
  ...
  async transferAsset(assetId: string, data: TransferAssetDTO) {
    // 1. Validate Asset
    const asset = await prisma.asset.findUnique({ where: { id: assetId } });
    if (!asset) {
      throw new AppError(`Asset with ID '${assetId}' not found`, 404);
    ...
    if (asset.status !== 'ASSIGNED') {
      throw new AppError(
        `Asset '${asset.assetCode}' must be in ASSIGNED status to be transferred (currently ${asset.status}).`,
        422,
      );
    }
    ...
    if (!newEmployee.isActive) {
      throw new AppError(
        `Employee '${newEmployee.firstName} ${newEmployee.lastName}' is inactive and cannot receive assets.`,
        422,
      );
    }
  ...
  async returnAsset(assetId: string, data: ReturnAssetDTO) {
    // 1. Validate Asset
    const asset = await prisma.asset.findUnique({ where: { id: assetId } });
    if (!asset) {
      throw new AppError(`Asset with ID '${assetId}' not found`, 404);
