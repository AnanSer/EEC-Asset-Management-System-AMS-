-- Phase 9A: Better Auth Foundation
-- Adds isolated auth tables alongside existing business tables.
-- The TESTING enum value was already applied directly to the DB in Phase 7;
-- we record it here so migrations stay in sync.

-- Fix schema drift: ensure TESTING enum value exists in MaintenanceStatus
ALTER TYPE "MaintenanceStatus" ADD VALUE IF NOT EXISTS 'TESTING';
