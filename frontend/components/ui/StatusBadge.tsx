...
const baseVariantMap: Record<string, { classes: string; dot: string; defaultLabel: string }> = {
  // Account Lifecycle Statuses (Phase 9B.1)
  ...
  verified:    { classes: 'bg-emerald-50 text-emerald-700 ring-emerald-200',   dot: 'bg-emerald-500', defaultLabel: 'Verified'          },
  unverified:  { classes: 'bg-slate-100  text-slate-600   ring-slate-200',     dot: 'bg-slate-400',   defaultLabel: 'Unverified'        },
  ...
  // Maintenance status
  open:        { classes: 'bg-sky-50     text-sky-700     ring-sky-200',       dot: 'bg-sky-500',     defaultLabel: 'Open'              },
  in_progress: { classes: 'bg-amber-50   text-amber-700   ring-amber-200',     dot: 'bg-amber-500',   defaultLabel: 'In Progress'       },
  completed:   { classes: 'bg-emerald-50 text-emerald-700 ring-emerald-200',   dot: 'bg-emerald-500', defaultLabel: 'Completed'         },
  on_hold:     { classes: 'bg-slate-100  text-slate-600   ring-slate-200',     dot: 'bg-slate-400',   defaultLabel: 'On Hold'           },
  cancelled:   { classes: 'bg-rose-50    text-rose-700    ring-rose-200',      dot: 'bg-rose-500',    defaultLabel: 'Cancelled'         },

  // Inspection results
  pass:        { classes: 'bg-emerald-50 text-emerald-700 ring-emerald-200',   dot: 'bg-emerald-500', defaultLabel: 'Passed'            },
  passed:      { classes: 'bg-emerald-50 text-emerald-700 ring-emerald-200',   dot: 'bg-emerald-500', defaultLabel: 'Passed'            },
  fail:        { classes: 'bg-rose-50    text-rose-700    ring-rose-200',      dot: 'bg-rose-500',    defaultLabel: 'Failed'            },
  failed:      { classes: 'bg-rose-50    text-rose-700    ring-rose-200',      dot: 'bg-rose-500',    defaultLabel: 'Failed'            },

  // Priorities
  low:         { classes: 'bg-slate-100  text-slate-600   ring-slate-200',     dot: 'bg-slate-400',   defaultLabel: 'Low'               },
  medium:      { classes: 'bg-blue-50    text-blue-700    ring-blue-200',      dot: 'bg-blue-500',    defaultLabel: 'Medium'            },
  high:        { classes: 'bg-orange-50  text-orange-700  ring-orange-200',    dot: 'bg-orange-500',  defaultLabel: 'High'              },
  critical:    { classes: 'bg-rose-50    text-rose-700    ring-rose-200',      dot: 'bg-rose-500',    defaultLabel: 'Critical'          },
};
...
export default function StatusBadge({ status, label, className }: StatusBadgeProps) {
  ...
  return (
    <span
      className={clsx(
        'inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold ring-1 ring-inset tracking-wide',
        classes,
        className
      )}
    >
